...
class PathSmoothing:
    ...
    def smooth_global_path(self, waypoints_array):
        ...
        for i in range(len(stop_line_index_old)):
            # find the index of the closest distance in the new distances array
            closest_index = np.argmin(np.abs(new_distances - distances[stop_line_index_old[i]]))
            # set the stop line id of the closest index to the stop line id of the old array
            stop_line_id_new[closest_index] = stop_line_id[stop_line_index_old[i]]
