...
class CameraTrafficLightDetector:
    ...
    def camera_image_callback(self, camera_image_msg):
        ...
        if stop_line_ids_on_path:
            ...
            rois = self.calculate_roi_coordinates(stop_line_ids_on_path, transform)
            ...
            if rois:
                ...
                for cl, (stop_line_id, traffic_light_id, _, _, _, _) in zip(classes, rois):

                    tfl_result = StopLineStatus()
                    tfl_result.traffic_light_id = traffic_light_id
                    tfl_result.stop_line_id = stop_line_id
                    tfl_result.status = CLASSIFIER_RESULT_TO_TLRESULT[cl]
                    tfl_result.status_text = CLASSIFIER_RESULT_TO_STRING[cl]
