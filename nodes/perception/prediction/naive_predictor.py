...
class NaivePredictor:
    ...
    def tracked_objects_callback(self, msg):
        ...
        for i, obj in enumerate(msg.objects):
            ...
            if get_speed_from_velocity(obj.velocity) < self.prediction_min_speed or obj.candidate_trajectories.paths:
                continue
        ...
        for i in valid_indices:
            ...
            for t in range(num_timesteps):
                wp = Waypoint()
                wp.position.x = float(predicted_positions[t, i, 0])
                wp.position.y = float(predicted_positions[t, i, 1])
                wp.position.z = obj_footprint_z
                wp.speed = float(np.linalg.norm(predicted_velocities[t, i]))
