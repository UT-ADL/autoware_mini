...
class Localizer:
    def __init__(self):
        ...
        if self.coordinate_transformer == "utm":
            self.transfromer = coordinate_transformations.WGS84ToUTMTransformer(self.use_custom_origin)
        elif self.coordinate_transformer == "lest97":
            self.transfromer = coordinate_transformations.WGS84ToLest97Transformer(self.use_custom_origin)
        else:
            rospy.logerr("novatel_localizer - coordinate_transformer not supported: %s ", str(self.coordinate_transformer))
            exit(1)
        ...
        rospy.loginfo("novatel_localizer - coordinate_transformer: %s ", str(self.coordinate_transformer))
        rospy.loginfo("novatel_localizer - use_msl_height: %s ", str(self.use_msl_height))
        rospy.loginfo("novatel_localizer - use_custom_origin: %s ", str(self.use_custom_origin))
...
if __name__ == '__main__':
    rospy.init_node('novatel_localizer', anonymous=True, log_level=rospy.INFO)
    node = Localizer()
    node.run()
