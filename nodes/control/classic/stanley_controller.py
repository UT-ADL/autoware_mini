...
import threading
import traceback
from autoware_mini.geometry import get_heading_from_orientation, get_heading_between_two_points, normalize_angle, get_point_using_heading_and_distance
...
class StanleyController:
    ...
    def current_status_callback(self, current_pose_msg, current_velocity_msg):
        ...
        try:
            ...
            lookback_on_path = path.linestring.interpolate(max(0.0, front_wheel_distance_from_path_start - self.wheel_base/10))
            track_heading = get_heading_between_two_points(lookback_on_path, lookahead_on_path)
            heading_error = normalize_angle(track_heading - current_heading)
            ...
            delta_error = math.atan(self.cte_gain * cross_track_error / (current_speed + 0.0001))
            ...
            if self.stanley_markers_pub.get_num_connections() > 0:
            ...
            if self.controller_debug_pub.get_num_connections() > 0:
                self.controller_debug_pub.publish(Float32MultiArray(data=[rospy.get_time() - start_time, current_heading, track_heading, heading_error, cross_track_error, target_speed]))
